...
class Importer:
    ...
    extended_material_names = False
    normalize_prefix = False
    normalize_prefix_root = False
    ...
    def execute(self):
        ...
        nif_mvp.discard_detached_skins(data)
        nif_mvp.repair_scene(data)
        nif_mvp.apply_hierarchy_scales(data)
            ...
            if self.process(node):
                ...
                if hasattr(node.source, "children"):
                    ...
                    if self.filter_best_lod and isinstance(node.source, nif.NiLODNode):
                        children = nif_mvp.best_lod_children(node.source, children)
    ...
    def apply_axis_corrections(self):
        ...
        for root_source in list(self.armatures):
            root = self.get(root_source)
            nif_mvp.apply_axis_corrections_one(root, list(self.iter_bones(root)), other_axis_correction)
...
class SceneNode:
    ...
    @property
    def name(self):
        return nif_mvp.sanitize_name(self.source.name, normalize_prefix=(self.importer.normalize_prefix and (self.parent is not None or self.importer.normalize_prefix_root)), normalize_prefix_max_length=self.importer.normalize_prefix_max_length)
...
class Material(SceneNode):
    ...
    def calc_name_from_textures(self, ni_prop, ni_alpha=None):
        ...
        # off, this reduces to the stock behaviour: texture stems only
        extended = self.importer.extended_material_names
        if ni_prop is None and not extended:
            return ""
        ...
        if ni_prop is not None:
            for tex_key, tex_map in zip(ni_prop.texture_keys, ni_prop.texture_maps):
                ...
                if extended and tex_key.startswith("decal_") and not tex_key.startswith("decal_0"):
                    continue
                try:
        ...
        ni_material = self.properties.get(nif.NiMaterialProperty) if extended else None
        if ni_material:
        ...
        if len(names) == 1 and "base_texture" in names:
            ...
            if texture_path:
                return nif_mvp.sanitize_name(f"{base} | path:{texture_path}", normalize_prefix=self.importer.normalize_prefix, normalize_prefix_max_length=self.importer.normalize_prefix_max_length)
            return base
        ...
        return nif_mvp.sanitize_name(final_name, normalize_prefix=self.importer.normalize_prefix, normalize_prefix_max_length=self.importer.normalize_prefix_max_length) if final_name else ""
