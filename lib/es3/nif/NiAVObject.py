...
from es3.utils.math import compose, decompose_uniform, dotproduct, ID33, ID44, ZERO3
...
class NiAVObject(NiObjectNET):
    ...
    def matrix_relative_to(self, ancestor: NiAVObject) -> ndarray:
        if self is ancestor:
            return ID44.copy()      # dotproduct([]) would raise: reduce, no initial
    ...
    def find_path(self, ancestor, breadth_first=True) -> Iterator[NiAVObject]:
        # Empty path, not a missing one; otherwise the search below looks for
        # `self` among its OWN descendants and raises.
        if self is ancestor:
            return
        parents = {}
